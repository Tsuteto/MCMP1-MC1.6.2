...
public class GuiRecorder extends GuiContainer
{
    ...
    {
        String s = StatCollector.translateToLocal(this.recorderInventory.getInvName());
        fontRenderer.drawString(s, this.xSize / 2 - this.fontRenderer.getStringWidth(s) / 2, 5, 4210752);
//        fontRenderer.drawString(StatCollector.translateToLocal("container.mcmp1.Recorder"), 60, 5, 0x404040);
        fontRenderer.drawString(StatCollector.translateToLocal("container.inventory"), 23, (ySize - 96) + 2, 0x404040);
    }
    ...
    protected void mouseClicked(int par1, int par2, int par3)
    {
        ...
        for (int l = 0; l < dispSonglist.size(); l++)
        {
            ...
            if (x >= 0 && y >= 0 && x < 78 && y < 12)
            {
                ...
                PacketDispatcher.sendPacketToServer(recorderInventory.dispatchControlPacket());
            }
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
    {
        ...
        for (int i = 0; i < dispSonglist.size(); i++)
        {
            ...
            if (McmpMain.useSmallFont)
            {
                GuiUtilities.showLongString(dispSonglist.get(i), ox + 12, oy + 18 + 12 * i, 78 - 4, color, fontRenderer);
            }
            else
            {
                GuiUtilities.showLongString1Line(dispSonglist.get(i), ox + 12, oy + 18 + 12 * i, 78 - 4, color, fontRenderer);
            }
        }
    }
}
