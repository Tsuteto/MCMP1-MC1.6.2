...
public class InventoryChanger extends InventoryBasic
{
    ...
    protected void dispatchStatePacket()
    {
    	...
    	ModLog.debug("LOC:" + locChanger);
    	if (locChanger < 0) return;
    	
		ModLog.debug("CHANGER-POS(%d, %d)", slotPlaying, playingInStack);
        CustomPacketDispatcher.create(McmpMain.netChannelChangerState)
		        .addBoolean(isPlayerControl)
		        .addInt(locChanger)
		        .addInt(slotPlaying)
		        .addInt(playingInStack)
		        .dispatch();
    }

    protected void dispatchRenamePacket(String var1)
    {
        CustomPacketDispatcher.create(McmpMain.netChannelChangerRename)
        .setString(var1)
        .dispatch();
    }
    ...
    public void readFromNBT(NBTTagCompound par1NBTTagCompound)
    {
        if (par1NBTTagCompound != null && par1NBTTagCompound.hasKey("McmpChanger"))
        {
            ...
            NBTTagList cassettes = changer.getTagList("Cassettes");

            for (int var3 = 0; var3 < cassettes.tagCount(); ++var3)
            {
                NBTTagCompound var4 = (NBTTagCompound) cassettes.tagAt(var3);
    ...
    public void writeToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        par1NBTTagCompound.setCompoundTag("McmpChanger", changer);
    }
